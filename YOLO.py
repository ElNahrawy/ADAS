...
class YOLOV4:
    def __init__(self,
                 cfg = 'utils/yolov4.cfg',
                 weights = 'weights/yolov4.weights',
                 use_gpu = False,
                 input_size = 512,
                 classes = 'utils/classes.names'):
        '''

        :param cfg: path to yolo cfg file
        :param weights: path to the yolo model weights
        :param use_gpu: if True use GPU, needs OpenCV supporting GPU to be installed
        :param input_size: yolo input size
        :param classes: path to file containing ordered class names
        '''
        self.cfg = cfg
    ...
    def image_inf(self, image, is_it_path = False, show_image = True, save_image = True):
        ...
        :param image: path to image or np array of the image
        :param is_it_path: True if image is a path not an array
        :param show_image: True to show inference results
        :param save_image: True to save image to result.jpg
        :return:
        '''
        if is_it_path:
    ...
    def return_boxes(self, image):
        ...
        :return: classes, confidences, bounding boxes detected by the object detector
        '''
        classes, confidences, boxes = self.net.detect(image, confThreshold=0.5, nmsThreshold=0.4)
